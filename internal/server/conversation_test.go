...
func TestIngestConversation_ConfidenceNormalized(t *testing.T) {
	tests := []struct {
		desc     string
		llmInput int // raw confidence from LLM (1-10 scale)
		wantMin  float64
		wantMax  float64
	}{
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			// Read confidence from stored point payload
...
func TestIngestConversation_FactsHaveNonZeroVectors(t *testing.T) {
	...
	for i, p := range mockStore.points {
		...
		if allZero {
			t.Errorf("point %d has all-zero vector", i)
		}

		// Also verify confidence is in 0-1 range, not raw 1-10
		confidenceVal := p.GetPayload()["confidence"]
		if confidenceVal != nil {
			c := confidenceVal.GetDoubleValue()
			if c < 0 || c > 1 {
				t.Errorf("point %d confidence = %f, expected in 0.0-1.0 range", i, c)
			}
		}
	}
}
...
func TestIngestConversation_EmbeddingFailureFallsBack(t *testing.T) {
	...
	mockEmbed := &mockEmbedder{embedErr: true}

	srv := &Server{
		facts:    mockStore,
		cfg:      &config.Config{FactsCollection: "test_facts", FactsVectorSize: 4},
		llm:      mockLLM,
		embedder: mockEmbed,
		logger:   testLogger(t),
	}
	...
	body := conversationRequest{
		Vault:    "default",
		Messages: []conversationMessage{{Role: "user", Content: "Test conversation"}},
	}
	bodyBytes, _ := json.Marshal(body)
	...
	if !allZero {
		t.Error("expected zero vector fallback on embedding failure")
	}
}

// ── Test: empty messages return error ─────────────────────────────────────────

func TestIngestConversation_EmptyMessages(t *testing.T) {
	body := conversationRequest{Vault: "default", Messages: []conversationMessage{}}
	bodyBytes, _ := json.Marshal(body)
